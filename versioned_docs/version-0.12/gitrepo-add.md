...
If you are using Fleet in a [single cluster](./concepts.md) style, the namespace will always be **fleet-local**. Refer to [cluster registration](./namespaces.md#cluster-registration-namespace-fleet-local) for more on the `fleet-local` namespace.
...
* from [SSH key fingerprints](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/githubs-ssh-key-fingerprints) for
Github
* from [SSH known hosts entries](https://docs.gitlab.com/ee/user/gitlab_com/index.html#ssh-known_hosts-entries) for Gitlab
...
* from [SSh keys to authenticate for azure devops](https://learn.microsoft.com/en-us/azure/devops/repos/git/use-ssh-keys-to-authenticate?view=azure-devops)
...
For more information, refer to [Fleet Troubleshooting documentation](./troubleshooting.md).
